...
int32_t der_count_children(const uint8_t *in, uint32_t in_data_len, uint32_t in_encoded_len, uint32_t *offset)
{
  uint32_t child_count = 0;
  uint32_t in_data_offset = 0;
  // Constructed type
  uint32_t children_length = 0;
  while (children_length < in_data_len)
  {
    const uint8_t *child = (in + in_data_offset);
    uint32_t child_max_length = in_encoded_len - in_data_offset;
    if (in_encoded_len < in_data_offset)
    ...
    if (child_length == 0xFFFFFFFF)
      ...
      return -1;
    if ((child_length + children_length) > in_encoded_len)
...
int32_t der_object_count(const uint8_t *in, uint32_t inlen)
{
  ...
  // types 0x03 and 0x04 can be used to encapsulate other ASN.1 types
  int encapsulating_maybe = (*in == 0x03) || (*in == 0x04);

  // An ASN.1 type is contructed, if bit 6 (from 1..8) is 1
  if ((*in & 0x20) != 0 || encapsulating_maybe)
  {
    int32_t children_count = der_count_children(in + in_data_offset, in_data_length, in_encoded_length, &in_data_offset);

    if (children_count < 0)
    {
      if (!encapsulating_maybe)
        // Error while counting the children
        return -1;
    }
    else
    {
      object_count += children_count;
    }
  }
...
int32_t der_decode(const uint8_t *in, uint32_t inlen, asn1_tree *out, asn1_tree *out_objects, unsigned int out_objects_count)
{
  ...
  if (*in == 0x03 || *in == 0x04)
  {
    encapsulating = der_count_children(in_data, in_data_length, in_encoded_length, NULL) > 0;
  }

  // An ASN.1 type is contructed, if bit 6 (from 1..8) is 1
  if ((*in & 0x20) != 0 || encapsulating)
  {
    // We have a constructed type
